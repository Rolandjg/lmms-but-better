...
public:
	...
	{
		auto point = std::lower_bound(points.begin(), points.end(), sampleOffset,
			[](const Point& p, int32 offset) { return p.offset < offset; });
		index = static_cast<int32>(point - points.begin());
		if (point != points.end() && point->offset == sampleOffset) { point->value = value; }
		else { points.insert(point, {sampleOffset, value}); }
		return kResultOk;
	}
};
...
void Vst3Plugin::parameterEditedByGui(Vst::ParamID id, double normalized)
{
	...
	if (it == m_paramById.end())
	{
		...
		m_pendingCc.push_back({id, normalized, 0});
		return;
	}
...
void Vst3Plugin::handleMidiInputEvent(const MidiEvent& event, const TimePos&, f_cnt_t offset)
{
	...
	{
		handleMidiInputEvent(MidiEvent{MidiEventTypes::MidiNoteOff, event.channel(), event.key(), 0}, {}, offset);
		return;
	}
...
void Vst3Plugin::process(const SampleFrame* in, SampleFrame* out, f_cnt_t frames)
{
...

	{
		...
		std::stable_sort(m_pendingEvents.begin(), m_pendingEvents.end(),
		...
		for (const auto& [id, value, offset] : m_pendingCc)
		{
			int32 index = 0;
			...
			{
				int32 pointIndex = 0;
				queue->addPoint(static_cast<int32>(std::min(offset, frames - 1)), value, pointIndex);
			}
		}
		m_pendingCc.clear();
	}
	...
	for (auto& bus : m_outBusBuffers) { bus.silenceFlags = 0; }
	...
	if (out)
	{
		if (result == kResultOk && m_mainOut >= 0 && m_outBuses[m_mainOut].channels > 0)
		{
			...
			for (f_cnt_t f = 0; f < frames; ++f)
			{
				const auto silence = m_outBusBuffers[m_mainOut].silenceFlags;
				out[f].setLeft((silence & 1) ? 0.f : left[f]);
				out[f].setRight((silence & (bus.channels > 1 ? 2 : 1)) ? 0.f : right[f]);
			}
		}
		else
		{
...
void Vst3Plugin::saveSettings(QDomDocument& doc, QDomElement& elem)
{
	...
	for (const auto& param : m_params)
	{
		if (param->model->isAutomatedOrControlled() || param->dirty.load()
...
void Vst3Plugin::loadSettings(const QDomElement& elem)
{
	...
	if (!params.isNull())
	{
		for (const auto& param : m_params)
		{
			const QString name = QString("param%1").arg(param->id);
			if (params.hasAttribute(name) || !params.firstChildElement(name).isNull())
			{
				param->model->loadSettings(params, name);
			}
		}
	}
}
