...
tresult PLUGIN_API MemoryStream::write(void* buffer, int32 numBytes, int32* numBytesWritten)
{
	if (numBytes < 0 || (!buffer && numBytes > 0)
		|| m_pos > std::numeric_limits<int>::max() - numBytes) { return kInvalidArgument; }
	if (m_pos + numBytes > m_data.size())
	{
		const auto oldSize = m_data.size();
		m_data.resize(static_cast<int>(m_pos + numBytes));
		std::memset(m_data.data() + oldSize, 0, m_data.size() - oldSize);
	}
	if (numBytes > 0) { std::memcpy(m_data.data() + m_pos, buffer, numBytes); }
	m_pos += numBytes;
...
tresult PLUGIN_API MemoryStream::seek(int64 pos, int32 mode, int64* result)
{
	...
	if (newPos < 0 || newPos > std::numeric_limits<int>::max()) { return kInvalidArgument; }
	m_pos = newPos;
